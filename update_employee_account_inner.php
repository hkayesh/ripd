...
elseif (isset($_POST['submit4'])) {
    ...
    for ($i = 1; $i < 9; $i++) {
        ...
        $scan_doc_name =  $file_name."-".$employeeID."-".$_FILES[$scanDoc]['name'];
        ...
        if (($_FILES[$scanDoc]['size'] < 999999999999) && in_array($extension, $allowedExts)) {
            ...
            $pathArray[$i] = "scaned/".$file_name."-".$employeeID."-".$p_scanname[$i];
        } else {
    ...
    $sql_images_scan_doc = mysql_query("INSERT INTO ep_certificate_scandoc_extra
                                 ...
                                 '$pathArray[5]',  '$pathArray[7]', '$pathArray[8]', 'emp','$employeeID')");
    }
 else {
        $sql_images_scan_doc = mysql_query("UPDATE ep_certificate_scandoc_extra SET emplo_scanDoc_national_id= '$pathArray[2]', emplo_scanDoc_birth_certificate='$pathArray[4]',
                                                ...
                                                scanDoc_hons='$pathArray[5]', scanDoc_masters=  '$pathArray[7]', scanDoc_other='$pathArray[8]' WHERE emp_type= 'emp' AND ep_id='$employeeID' ") or exit(mysql_error());
    }
